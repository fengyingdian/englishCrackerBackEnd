...
class work extends io
{
    ...
    public function get_account_state(){
        ...
        if ($rows) {
            ...
            $result["data"]["goods_id"]      = $chapters_info["goods_id"];
            if ($rows["start_time"] < time()) {
                $result["data"]["state"] = 2; //已购买课程已开始
            } else {
                $result["data"]["state"] = 3; //已购买课程未开始
            }
        } else {
    ...
    public function get_start_course_days(){
        ...
        if ($goods_info){
            $result["data"]["days"] = intval((time()-$goods_info["start_time"])/86400);
        }
